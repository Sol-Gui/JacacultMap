...
const Header: React.FC<HeaderProps> = ({ onMenuPress, theme, isDarkMode, onThemeToggle, showMenuButton = true }) => {
  ...
  return (
    <View
      className="mb-4 flex-row items-center justify-between border-b px-4 py-3 md:px-8"
    ...
    >
      ...
      <View className="flex-row items-center gap-3">
        <TouchableOpacity className="rounded-full p-2" style={{ backgroundColor: theme.background }} onPress={onThemeToggle} accessibilityLabel="Alternar tema">
          <Feather 
        ...
        <TouchableOpacity onPress={() => router.navigate('/(tabs)/profile')} accessibilityLabel="Abrir perfil">
          <Image source={{ uri: userImage }} className="h-10 w-10 rounded-full border-2 border-[#A3E635]" />
        </TouchableOpacity>
      </View>
    </View>
  );
